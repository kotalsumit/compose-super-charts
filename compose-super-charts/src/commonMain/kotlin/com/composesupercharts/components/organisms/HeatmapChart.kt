...
fun HeatmapChart(
    ...
    Column(modifier = modifier.padding(config.padding).semantics { heatmapDescription(data) }) {
        BoxWithConstraints(modifier = Modifier.weight(1f)) {
            ...
            val cellWidth =
                (width - (cols - 1) * with(density) { config.cellSpacing.toPx() }) / cols
            val cellHeight =
                (height - (rows - 1) * with(density) { config.cellSpacing.toPx() }) / rows
            ...
            Box(
                ...
                Column {
                    Canvas(
                        modifier = Modifier
                            .requiredWidth(with(density) { (cols * (cellSize + with(density) { config.cellSpacing.toPx() })).toDp() })
                            .requiredHeight(with(density) { (rows * (cellSize + with(density) { config.cellSpacing.toPx() })).toDp() })
                            .pointerInput(data) {
                                awaitEachGesture {
                                    var zoom = 1f
                                    ...
                                    do {
                                        ...
                                        if (isZooming) {
                                            ...
                                            if (!pastTouchSlop) {
                                                ...
                                                val zoomMotion =
                                                    kotlin.math.abs(1 - zoom) * centroidSize
                            ...
                            .pointerInput(data) {
                                detectTapGestures { tap ->
                                    val col =
                                        (tap.x / (cellSize + with(density) { config.cellSpacing.toPx() })).toInt()
                                    val row =
                                        (tap.y / (cellSize + with(density) { config.cellSpacing.toPx() })).toInt()
                    ...
                    TooltipBubble(
                        ...
                        labels = listOf(
                            TooltipBubbleData(
                                ...
                                value = cell.label ?: cell.value.toString()
                            )
                        ),
                        isFirst = cell.col == 0,
                        isLast = cell.col == (data.cells.maxOfOrNull { it.col } ?: 0),
